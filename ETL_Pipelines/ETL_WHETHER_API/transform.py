...
RAW_DIR = BASE_DIR / "data" / "raw"   #raw → untouched API data
print("Raw_dir: ", RAW_DIR)

STAGED_DIR = BASE_DIR / "data" / "staged"   #staged → transformed, cleaned data ready for loading  ,cleaned & structured but not final
PROCESSED_DIR = BASE_DIR / "data" / "processed"  #processed → analysis outputs, plots, reports
...
def _flatten_weather_json(json_path: str) -> pd.DataFrame:
    ...
    #Reads the raw file into a Python dictionary.
    with open(json_path, "r") as f:          # Open the JSON file for reading
        payload = json.load(f)               # Load JSON data from file
